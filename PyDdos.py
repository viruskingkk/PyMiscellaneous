	...
	def doconnection(self):
		...
		while socks < int(self.threads):
			try:
				...
				break
				sys.exit(cprint('[-] Canceled by user','red'))
		cprint('I have sent ','green') + cprint(str(self.pkt_count),'cyan') + cprint(' packets successfully.Now i\'m going to sleep for ','green') + cprint(self.sleep,'red') + cprint(' second','green')
...
'''
	...
	if args.Synflood:
		...
		ts=[]
		threads=[]
		cprint('[*] Started SYN Flood: ','blue')+cprint(tgt,'red')
		while 1:
			if args.i == False:
				args.fakeip = True
				if args.fakeip == True:
		...
		tgt = args.d
		threads = []
		cprint('[*] Start send request to: ','blue')+cprint(tgt,'red')
		while 1:
			try:
